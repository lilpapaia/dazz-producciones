/**
 * DEUDA-L1: Logo Dazz reutilizable (asterisco).
 * @param {number} size - Width/height in px (default 32)
 * @param {string} className - Additional classes (default text-amber-500)
 */
const DazzLogo = ({ size = 32, className = 'text-amber-500' }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 66 69" fill="none" className={className}>
    <path d="M58.7442 59.5633L46.4651 68.332L32.7907 50.377L19.5349 68.332L6.97674 59.5633L20.3721 40.634L0 34.2314L4.60465 20.1736L24.5581 26.5761V3.33203H41.0233V26.5761L60.9767 20.1736L66 34.2314L45.3488 40.634L58.7442 59.5633Z" fill="currentColor"/>
  </svg>
);

export default DazzLogo;
