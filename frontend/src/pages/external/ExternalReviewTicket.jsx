...
import { getGuestTickets, updateGuestTicket, deleteGuestTicket, getGuestProject } from '../../services/shareApi';
...
// FEAT-09: copia de ReviewTicket para el flujo externo. El externo nunca es admin;
// usa shareApi; sin estadísticas; los tickets de proveedor son de solo lectura y no
// se pueden borrar desde aquí.
const ExternalReviewTicket = () => {
  const { token, ticketId } = useParams();
  ...
  const loadTicket = async () => {
    try {
      // El backend no expone un ticket individual para el guest → traemos la lista
      // del proyecto (del JWT) y filtramos por ticketId.
      const [ticketsRes, projectRes] = await Promise.all([
        getGuestTickets(),
        getGuestProject().catch(() => null),
      ]);
      const all = ticketsRes.data;
      const currentTicket = all.find(t => t.id === parseInt(ticketId));

      if (!currentTicket) {
        showError('Ticket no encontrado');
        navigate(`/share/${token}/project`);
        return;
      }

      setTicket(currentTicket);
      initialTicketRef.current = JSON.stringify(currentTicket);
      if (projectRes) setProjectStatus(projectRes.data.status);

      setAllTickets(all);
      setCurrentTicketIndex(all.findIndex(t => t.id === parseInt(ticketId)));

      if (currentTicket.invoice_number) {
        const dup = all.find(t => t.id !== currentTicket.id && t.invoice_number === currentTicket.invoice_number);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await updateGuestTicket(ticketId, { ...ticket, is_reviewed: true });
      showSuccess('Ticket actualizado y marcado como revisado');
      navigate(`/share/${token}/project`);
    } catch (error) {
      showError(error.response?.data?.detail || 'Error al actualizar ticket');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await deleteGuestTicket(ticketId);
  ...
  const getPages = () => {
    ...
    if (ticket.file_pages) {
      if (Array.isArray(ticket.file_pages)) return ticket.file_pages;
  ...
  const isProjectClosed = projectStatus === 'cerrado';
  const supplierFieldsDisabled = isSupplierTicket; // el externo nunca es admin
  ...
  const goToPrevTicket = () => { if (hasPrevTicket) navigate(`/share/${token}/ticket/${allTickets[currentTicketIndex - 1].id}`); };
  const goToNextTicket = () => { if (hasNextTicket) navigate(`/share/${token}/ticket/${allTickets[currentTicketIndex + 1].id}`); };
  ...
  const handleLateralDoubleClick = (side) => {
    if (side === 'left' && hasPrevTicket) goToPrevTicket();
    else if (side === 'right' && hasNextTicket) goToNextTicket();
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4">
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            {ticket.uploaded_by_guest_name && (
              <span className="bg-teal-500/20 text-teal-400 border border-teal-500/30 px-2 py-0.5 rounded-sm text-xs font-bold">EXTERNO — {ticket.uploaded_by_guest_name}</span>
            )}
            {ticket.is_foreign && (
              <span className="bg-blue-500/20 text-blue-400 px-3 py-1 rounded-sm text-xs font-bold uppercase border border-blue-500/30 flex items-center gap-1">🌍 INTERNACIONAL</span>
            )}
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            {/* Eliminar — cerrado/PAGADO disabled; proveedor oculto; normal activo */}
            {isProjectClosed ? (
              <button
                onClick={() => showError('El proyecto está cerrado')}
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 pt-6 pb-8 relative">
        {hasPrevTicket && (
          <div onDoubleClick={() => handleLateralDoubleClick('left')} className="fixed left-0 top-0 bottom-0 w-10 z-30 cursor-pointer" title="Doble click para ticket anterior" />
        )}
        {hasNextTicket && (
          <div onDoubleClick={() => handleLateralDoubleClick('right')} className="fixed right-0 top-0 bottom-0 w-10 z-30 cursor-pointer" title="Doble click para siguiente ticket" />
        )}

        {hasPrevTicket && (
          <button onClick={goToPrevTicket} className="hidden md:flex fixed left-4 top-1/2 -translate-y-1/2 z-40 bg-zinc-800/90 hover:bg-zinc-700 text-zinc-300 hover:text-white p-3 rounded-full border border-zinc-600 items-center justify-center transition-all shadow-lg" title="Ticket anterior">
            <ChevronLeft size={24} />
          </button>
        )}
        {hasNextTicket && (
          <button onClick={goToNextTicket} className="hidden md:flex fixed right-4 top-1/2 -translate-y-1/2 z-40 bg-zinc-800/90 hover:bg-zinc-700 text-zinc-300 hover:text-white p-3 rounded-full border border-zinc-600 items-center justify-center transition-all shadow-lg" title="Siguiente ticket">
        ...
        {duplicateWarning && (
          <div className="mb-4 bg-amber-500/10 border-2 border-amber-500/40 rounded-sm p-4 flex items-center justify-between gap-4">
            ...
            <button
              onClick={() => navigate(`/share/${token}/ticket/${duplicateWarning.ticket_id}`)}
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-sm p-6 space-y-6">
          ...
          {pages.length > 0 && (
            <div className="bg-zinc-950 border border-zinc-700 rounded-sm overflow-hidden">
              {isPdfFallback ? (
                <iframe src={pages[0]} title="PDF" className="w-full h-96 bg-zinc-900" />
              ) : (
              ...
              <div className="px-4 py-3 bg-zinc-900/80 border-t border-zinc-800 flex items-center justify-center">
                <div className="flex items-center gap-3">
                  <button onClick={() => setCurrentPage(p => Math.max(0, p - 1))} disabled={currentPage === 0 || totalPages <= 1} className="p-1.5 rounded-sm bg-zinc-800 hover:bg-zinc-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors flex-shrink-0">
                    <ChevronLeft size={18} />
                  </button>
                  <span className="text-sm font-mono text-zinc-300 text-center">
                    {totalPages > 1 ? `${currentPage + 1}/${totalPages} - ${getTruncatedFileName(ticket.file_name)}` : getTruncatedFileName(ticket.file_name)}
                  </span>
                  <button onClick={() => setCurrentPage(p => Math.min(totalPages - 1, p + 1))} disabled={currentPage === totalPages - 1 || totalPages <= 1} className="p-1.5 rounded-sm bg-zinc-800 hover:bg-zinc-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors flex-shrink-0">
          ...
          {showLightbox && pages.length > 0 && !isPdfFallback && (
            <div className="fixed inset-0 !m-0 bg-black z-50 flex items-center justify-center" style={{ minHeight: '100dvh', paddingTop: 'env(safe-area-inset-top, 0px)', paddingBottom: 'env(safe-area-inset-bottom, 0px)' }} onClick={() => setShowLightbox(false)}>
              <button onClick={(e) => { e.stopPropagation(); setShowLightbox(false); }} className="absolute top-4 right-4 text-white hover:text-amber-500 transition-colors bg-zinc-900/80 rounded-full p-2 border border-zinc-700 z-10" style={{ marginTop: 'env(safe-area-inset-top, 0px)' }}>
                <X size={32} />
              </button>
              {totalPages > 1 && currentPage > 0 && (
                <button onClick={(e) => { e.stopPropagation(); setCurrentPage(p => p - 1); }} className="hidden md:flex absolute left-4 top-1/2 -translate-y-1/2 bg-zinc-900/80 hover:bg-zinc-700 text-white p-3 rounded-full border border-zinc-700 items-center justify-center">
                  <ChevronLeft size={28} />
                </button>
              )}
              <img src={pages[currentPage]} alt={`Página ${currentPage + 1}`} className="max-w-full max-h-[90vh] object-contain shadow-2xl select-none" onClick={(e) => e.stopPropagation()} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} />
              {totalPages > 1 && currentPage < totalPages - 1 && (
                <button onClick={(e) => { e.stopPropagation(); setCurrentPage(p => p + 1); }} className="hidden md:flex absolute right-4 top-1/2 -translate-y-1/2 bg-zinc-900/80 hover:bg-zinc-700 text-white p-3 rounded-full border border-zinc-700 items-center justify-center">
          ...
          {/* Alerta IA */}
          <div className={`rounded-sm p-4 ${isExtractionFailed ? 'bg-red-500/10 border border-red-500/30' : ticket.ia_warnings ? 'bg-amber-500/10 border border-amber-500/30' : 'bg-blue-500/10 border border-blue-500/30'}`}>
            <div className="flex items-start gap-3">
              <div className={`mt-0.5 ${isExtractionFailed ? 'text-red-400' : ticket.ia_warnings ? 'text-amber-400' : 'text-blue-400'}`}>
              ...
              <div>
                <p className={`text-sm font-semibold ${isExtractionFailed ? 'text-red-400' : ticket.ia_warnings ? 'text-amber-400' : 'text-blue-400'}`}>
          ...
          {/* Banner ticket proveedor (solo lectura) */}
          {isSupplierTicket && (
            <div className="bg-purple-500/10 border-2 border-purple-500/30 rounded-sm p-4">
              <div className="flex items-center gap-2">
                <span className="text-lg">🔗</span>
                <p className="text-sm text-purple-300">Esta factura proviene del portal de proveedores. Los importes no son editables.</p>
              </div>
            </div>
          )}
          ...
          {ticket.is_foreign && (
            <div className="bg-blue-500/10 border-2 border-blue-500/30 rounded-sm p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div><p className="text-zinc-500 text-xs mb-1">País</p><p className="font-semibold">{ticket.country_code || 'N/A'}</p></div>
                <div><p className="text-zinc-500 text-xs mb-1">Divisa</p><p className="font-semibold"><span className="bg-amber-500/20 text-amber-400 px-2 py-1 rounded text-sm">{getCurrencySymbol(ticket.currency)} {ticket.currency}</span></p></div>
                <div><p className="text-zinc-500 text-xs mb-1">Clasificación</p><span className={`px-2 py-1 rounded text-xs ${ticket.geo_classification === 'UE' ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}`}>{ticket.geo_classification || 'N/A'}</span></div>
                {ticket.exchange_rate && (<div><p className="text-zinc-500 text-xs mb-1">Tasa cambio</p><p className="font-semibold">{ticket.exchange_rate.toFixed(4)}</p></div>)}
              </div>
              {ticket.foreign_amount && (
                <div className="mt-4 pt-4 border-t border-blue-500/30">
                  <p className="text-xs text-zinc-500 mb-3">Importes en divisa original:</p>
                  <div className="flex flex-wrap gap-4 text-sm">
                    <div><span className="text-zinc-400 text-xs">Base: </span><span className="font-semibold text-blue-400">{getCurrencySymbol(ticket.currency)}{ticket.foreign_amount.toFixed(2)}</span></div>
                    {ticket.foreign_tax_amount && (<><span className="text-zinc-600">|</span><div><span className="text-zinc-400 text-xs">IVA: </span><span className="font-semibold text-blue-400">{getCurrencySymbol(ticket.currency)}{ticket.foreign_tax_amount.toFixed(2)}</span></div></>)}
                    {ticket.foreign_tax_eur && (<><span className="text-zinc-600">|</span><div><span className="text-zinc-400 text-xs">IVA reclamable: </span><span className="font-bold text-green-400">{ticket.foreign_tax_eur.toFixed(2)}€ ✅</span></div></>)}
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={ticket.date || ''} onChange={(e) => setTicket({...ticket, date: e.target.value})} disabled={isSupplierTicket} placeholder="DD/MM/AAAA" className="w-full bg-zinc-950 border border-zinc-700 rounded-sm px-4 py-2.5 text-zinc-100 focus:outline-none focus:border-amber-500 disabled:opacity-60 disabled:cursor-not-allowed" />
            </div>
            {ticket.type === 'factura' && (
              <div>
                <label className="block text-xs font-mono text-zinc-400 mb-2 tracking-wider">Nº FACTURA</label>
                <input type="text" value={ticket.invoice_number || ''} onChange={(e) => setTicket({...ticket, invoice_number: e.target.value})} disabled={isSupplierTicket} className="w-full bg-zinc-950 border border-zinc-700 rounded-sm px-4 py-2.5 text-zinc-100 focus:outline-none focus:border-amber-500 disabled:opacity-60 disabled:cursor-not-allowed" />
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <input type="text" value={ticket.provider || ''} onChange={(e) => setTicket({...ticket, provider: e.target.value})} disabled={isSupplierTicket} className="w-full bg-zinc-950 border border-zinc-700 rounded-sm px-4 py-2.5 text-zinc-100 focus:outline-none focus:border-amber-500 disabled:opacity-60 disabled:cursor-not-allowed" />
          </div>
          ...
          {ticket.type === 'factura' && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={ticket.phone || ''} onChange={(e) => setTicket({...ticket, phone: e.target.value})} disabled={isSupplierTicket} className="w-full bg-zinc-950 border border-zinc-700 rounded-sm px-4 py-2.5 text-zinc-100 focus:outline-none focus:border-amber-500 disabled:opacity-60 disabled:cursor-not-allowed" />
              </div>
              <div>
                <label className="block text-xs font-mono text-zinc-400 mb-2 tracking-wider">EMAIL</label>
                <input type="email" value={ticket.email || ''} onChange={(e) => setTicket({...ticket, email: e.target.value})} disabled={isSupplierTicket} className="w-full bg-zinc-950 border border-zinc-700 rounded-sm px-4 py-2.5 text-zinc-100 focus:outline-none focus:border-amber-500 disabled:opacity-60 disabled:cursor-not-allowed" />
              </div>
            </div>
          )}
          ...
          {ticket.type === 'factura' && (
            <div>
              ...
              <input type="text" value={ticket.contact_name || ''} onChange={(e) => setTicket({...ticket, contact_name: e.target.value})} disabled={isSupplierTicket} className="w-full bg-zinc-950 border border-zinc-700 rounded-sm px-4 py-2.5 text-zinc-100 focus:outline-none focus:border-amber-500 disabled:opacity-60 disabled:cursor-not-allowed" />
            </div>
          )}
            ...
            {ticket.is_foreign && ticket.currency !== 'EUR' ? (
              <>
                <div className="flex items-center justify-between mb-4">
                  ...
                  {ticket.exchange_rate && (<span className="text-[10px] text-zinc-600 font-mono bg-zinc-800 px-2.5 py-1 rounded">1 {ticket.currency} = {ticket.exchange_rate.toFixed(4)} EUR</span>)}
                </div>
                ...
                {(() => {
                  ...
                  const updateFromForeign = (foreignBase, ivaPct) => {
                    ...
                    setTicket({
                      ...ticket,
                      foreign_amount: foreignBase, foreign_tax_amount: foreignTax, foreign_total: foreignTotal,
                    ...
                    <>
                      <div className="hidden md:grid grid-cols-[1fr_1px_1fr] gap-0">
                        ...
                        <div className="flex items-center justify-between py-2.5 border-b border-zinc-800/50 pr-5">
                          ...
                          <div className="relative w-28">
                            <AmountInput value={ticket.foreign_amount} onCommit={(n) => updateFromForeign(n ?? 0, ticket.iva_percentage || 0)} disabled={isSupplierTicket} className={`${inputCls} text-right pr-7 text-sm`} />
                        ...
                        <div className="flex items-center justify-between py-2.5 border-b border-zinc-800/50 pr-5">
                          ...
                          <div className="flex items-center gap-2">
                            <div className="relative w-16">
                              <AmountInput decimals={0} min={0} max={100} value={ticket.iva_percentage != null ? Math.round(ticket.iva_percentage * 100) : null} onCommit={(n) => updateFromForeign(ticket.foreign_amount || 0, (n ?? 0) / 100)} disabled={isSupplierTicket} className={`${inputCls} text-center pr-6 text-sm`} />
                      ...
                      <div className="md:hidden space-y-4">
                        ...
                        <div className="border-t border-zinc-700 pt-4">
                          ...
                          <div className="flex items-center justify-between py-1.5"><span className="text-[13px] text-zinc-500">Base</span><span className="text-sm text-zinc-400 font-mono">≈ {ticket.base_amount?.toFixed(2)} €</span></div>
                          <div className="flex items-center justify-between py-1.5"><span className="text-[13px] text-zinc-500">IVA</span><span className="text-sm text-zinc-400 font-mono">≈ {ticket.iva_amount?.toFixed(2)} €</span></div>
                          <div className="flex items-center justify-between pt-2 mt-1 border-t border-zinc-800/50"><span className="text-[13px] text-zinc-400">Total</span><span className="text-lg text-zinc-300 font-medium font-mono">≈ {ticket.final_total?.toFixed(2)} €</span></div>
                        </div>
                      </div>
                    </>
                  );
              ...
              <>
                ...
                {(ticket.irpf_amount > 0) && (
                  <div className="mb-2 text-xs text-zinc-500">IRPF: -{ticket.irpf_amount?.toFixed(2)}€</div>
                )}
          ...
          <div>
            ...
            <textarea value={ticket.notes || ''} onChange={(e) => setTicket({...ticket, notes: e.target.value})} rows={3} placeholder="Descripción del gasto, observaciones..." className="w-full bg-zinc-950 border border-zinc-700 rounded-sm px-4 py-2.5 text-zinc-100 focus:outline-none focus:border-amber-500 resize-none" />
          </div>

          {/* Gasto suplido — solo tickets internos */}
          {!ticket.from_supplier_portal && (
            <label className="flex items-center gap-3 cursor-pointer group">
              <input type="checkbox" checked={ticket.is_suplido || false} onChange={(e) => setTicket({...ticket, is_suplido: e.target.checked})} className="w-4 h-4 rounded border-zinc-600 bg-zinc-950 text-amber-500 focus:ring-amber-500 focus:ring-offset-0 cursor-pointer" />
              <div>
          ...
          <div className={customPayment ? "flex flex-col gap-4" : "grid grid-cols-2 gap-4"}>
            <div>
              ...
              <select value={ticket.invoice_status || ''} onChange={(e) => setTicket({...ticket, invoice_status: e.target.value})} disabled={supplierFieldsDisabled} className={`w-full bg-zinc-950 border border-zinc-700 rounded-sm px-4 py-2.5 text-zinc-100 focus:outline-none focus:border-amber-500${supplierFieldsDisabled ? ' opacity-60 cursor-not-allowed' : ''}`}>
            ...
            <div>
              ...
              {!customPayment ? (
                <select
                  ...
                  onChange={(e) => {
                    if (e.target.value === '__custom__') { setCustomPayment(true); setTicket({...ticket, payment_status: ''}); }
                    else { setTicket({...ticket, payment_status: e.target.value}); }
                  }}
                ...
                <div className="flex gap-2 items-center">
                  <input type="text" autoFocus disabled={supplierFieldsDisabled} value={ticket.payment_status || ''} onChange={(e) => setTicket({...ticket, payment_status: e.target.value})} onKeyDown={(e) => { if (e.key === 'Enter') e.preventDefault(); }} placeholder="Escribe el estatus de pago..." className={`flex-1 min-w-0 bg-zinc-950 border border-amber-500 rounded-sm px-3 py-2.5 text-zinc-100 focus:outline-none text-sm${supplierFieldsDisabled ? ' opacity-60 cursor-not-allowed' : ''}`} />
                  <button onClick={() => { setCustomPayment(false); setTicket({...ticket, payment_status: ''}); }} className="flex-shrink-0 px-3 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-zinc-100 rounded-sm transition-colors text-sm" title="Volver al desplegable">↩</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-4">
            <button onClick={() => navigate(`/share/${token}/project`)} className="flex-1 px-6 py-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-100 rounded-sm transition-colors font-semibold">Cancelar</button>
            <button onClick={handleSave} disabled={saving} className="flex-1 px-6 py-3 bg-amber-500 hover:bg-amber-600 text-zinc-950 font-bold rounded-sm transition-all shadow-lg shadow-amber-500/30 disabled:opacity-50 flex items-center justify-center gap-2">
              <Save size={18} />
...
export default ExternalReviewTicket;
