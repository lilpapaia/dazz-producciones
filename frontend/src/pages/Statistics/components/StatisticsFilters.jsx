...
const StatisticsFilters = memo(({
  ...
  const showCompanySelector = (userRole === ROLES.ADMIN || userRole === ROLES.BOSS) && companies.length > 0;

  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-sm p-6">
      <div className={`grid grid-cols-1 ${showCompanySelector ? 'md:grid-cols-4' : 'md:grid-cols-3'} gap-4`}>
        ...
        {showCompanySelector && (
          <div>
            ...
            >
              <option value="">{userRole === ROLES.ADMIN ? 'TODAS LAS EMPRESAS' : 'TODAS MIS EMPRESAS'}</option>
              {companies.map(c => (
