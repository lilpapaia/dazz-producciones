...
const Statistics = () => {
  ...
  const exportPDFReport = () => {
    ...
    drawBox(0, 0, pageWidth, 45, [24, 24, 27]);
    ...
    checkPageBreak(45);
    drawBox(15, yPos - 5, pageWidth - 30, 40, [254, 243, 199]);
    doc.setFontSize(14);
    doc.setFont(undefined, 'bold');
    ...
    resumeItems.forEach(item => { doc.text(item, 25, yPos); yPos += lineHeight + 1; });
    yPos += 15;

    foreign_breakdown.forEach((country) => {
      checkPageBreak(60);
      drawBox(15, yPos - 5, pageWidth - 30, 12, [219, 234, 254]);
      doc.setFontSize(13);
      doc.setFont(undefined, 'bold');
      ...
      // Para el PDF, obtener todos los proyectos (de companies o de projects directamente)
      const allProjects = country.companies
        ? country.companies.flatMap(c => c.projects)
        : (country.projects || []);
      ...
      allProjects.forEach((project) => {
        ...
        if (tickets.length > 0) {
          ...
          doc.text(`> TICKETS INTERNACIONALES (${tickets.length}):`, 25, yPos);
          yPos += 7;
          doc.setTextColor(0, 0, 0);

          tickets.forEach((ticket, index) => {
            checkPageBreak(30);
            drawBox(28, yPos - 3, pageWidth - 56, 25, [249, 250, 251]);
            doc.setFontSize(9);
            doc.setFont(undefined, 'bold');
            ...
            ticketInfo.forEach(info => { doc.text(info, 35, yPos); yPos += 5; });
            doc.setFont(undefined, 'bold');
            doc.setTextColor(...colors.success);
