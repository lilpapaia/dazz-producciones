    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        <div className="flex gap-2">
          {unreadCount > 0 && (
            <button onClick={handleMarkAllRead} className="text-[13px] text-zinc-400 border border-zinc-700 px-3 py-1.5 rounded hover:bg-zinc-800 transition-colors">
              Marcar todo leído
            </button>
          )}
        </div>
      ...
      <div className="flex gap-2 mb-3.5 items-center">
        <button onClick={() => setFilter('all')} className={`text-[13px] px-3 py-1 rounded-full border transition-all ${filter === 'all' ? 'bg-amber-500 text-zinc-950 border-amber-500 font-semibold' : 'border-zinc-700 text-zinc-400'}`}>Todas</button>
        <button onClick={() => setFilter('unread')} className={`text-[13px] px-3 py-1 rounded-full border transition-all ${filter === 'unread' ? 'bg-amber-500 text-zinc-950 border-amber-500 font-semibold' : 'border-zinc-700 text-zinc-400'}`}>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-md overflow-hidden">
        {/* Count bar (N4) */}
        <div className="flex items-center justify-between px-3.5 py-2 bg-zinc-800 text-[13px] text-zinc-400 border-b border-white/[.04]">
              ...
              >
                ...
                {hasSupplier && (
                  <button
                    ...
                    className="text-[13px] text-zinc-500 hover:text-zinc-300 border border-zinc-700 px-2 py-1 rounded flex items-center gap-0.5 flex-shrink-0 transition-colors"
                  >
      ...
      {visibleCount < notifications.length && (
        <div className="text-center mt-3">
          <button onClick={() => setVisibleCount(v => v + 50)} className="text-[13px] text-amber-500 hover:text-amber-400">
