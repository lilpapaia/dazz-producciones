...
async def generate_autoinvoice(
    ...
    # C-1: Advisory lock to serialize autoinvoice number generation per company.
    # Prevents race condition where 2 admins get the same invoice number.
    is_postgres = str(db.bind.url).startswith("postgresql") if db.bind else False
    if is_postgres:
        db.execute(text("SELECT pg_advisory_xact_lock(:lock_id)"), {"lock_id": 200000 + body.company_id})

    # Regenerate invoice number server-side inside the lock (ignore client-supplied number)
    year = datetime.now(timezone.utc).year
    latest = db.query(SupplierInvoice.invoice_number).filter(
        SupplierInvoice.company_id == body.company_id,
        SupplierInvoice.is_autoinvoice == True,
        SupplierInvoice.invoice_number.like(f"%-{year}-%"),
    ).order_by(desc(SupplierInvoice.created_at)).first()
    ...
    # Safety check: if somehow the number still exists, fail fast
    existing = db.query(SupplierInvoice.id).filter(
        SupplierInvoice.invoice_number == invoice_number,
    ).first()
    if existing:
        raise HTTPException(409, f"El número de factura '{invoice_number}' ya está registrado en el sistema.")
    ...
    _notify(db, NotificationRecipientType.SUPPLIER, supplier.id,
            ...
            f"{invoice_number} — {final_total:.2f} EUR generated by {company.name}",
    ...
    _notify(db, NotificationRecipientType.ADMIN, ADMIN_RECIPIENT_ID,
            ...
            f"{invoice_number} for {supplier.name} — {final_total:.2f} EUR",
            invoice_id=invoice.id, supplier_id=supplier.id)
    try:
        db.commit()
    except Exception:
    ...
    try:
        send_autoinvoice_notification(
            ...
            invoice_number, final_total, company.name
        )
    except Exception as e:
