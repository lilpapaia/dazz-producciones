...
def upload_ticket_file(file_path: str, file_name: str, project_id: int) -> dict:
    ...
    try:
        if is_image:
            ...
            # Subir PDF sin transformación
            result = cloudinary.uploader.upload(
                file_path,
                public_id=public_id,
                resource_type="raw",     # PDF como raw file
                overwrite=True
            )
        else:
            # Otros tipos como imagen genérica
            result = cloudinary.uploader.upload(
                file_path,
                public_id=public_id,
                resource_type="auto",
