...
def get_styled_email_template(project_name, project_code, responsible_name, tickets_count, total_amount, excel_filename):
    """Template de email con estilo de la app (dark theme, amber/zinc)"""
...
<head>
    ...
    <style>
        ...
        .header h1 {{margin: 10px 0 0 0; font-size: 32px; font-weight: 900; letter-spacing: 0.05em; color: #18181b; text-transform: uppercase;}}
        .content {{background-color: #27272a; margin: 20px; padding: 30px; border-radius: 8px; border: 1px solid #3f3f46;}}
        .intro {{color: #a1a1aa; font-size: 15px; line-height: 1.6; margin-bottom: 25px;}}
        .info-grid {{background-color: #18181b; border: 1px solid #3f3f46; border-radius: 6px; padding: 20px; margin: 20px 0;}}
        .info-row {{display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid #3f3f46;}}
        .info-row:last-child {{border-bottom: none;}}
        .info-label {{color: #71717a; font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;}}
        .info-value {{color: #f4f4f5; font-size: 15px; font-weight: 600; text-align: right;}}
        .total-amount {{font-size: 28px; font-weight: 900; color: #f59e0b;}}
        .attachment-box {{background: linear-gradient(135deg, #f59e0b15 0%, #d9770615 100%); border: 2px solid #f59e0b30; border-radius: 8px; padding: 20px; margin: 25px 0; text-align: center;}}
        .attachment-icon {{font-size: 40px; margin-bottom: 10px;}}
        .attachment-title {{color: #f59e0b; font-size: 16px; font-weight: 700; margin-bottom: 8px;}}
        .attachment-filename {{color: #a1a1aa; font-size: 14px; font-family: 'Courier New', monospace;}}
        .footer {{text-align: center; padding: 30px 20px; color: #71717a; font-size: 13px; line-height: 1.6;}}
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <svg xmlns="http://www.w3.org/2000/svg" width="140" height="14" viewBox="0 0 708 69" fill="none" style="margin-bottom: 10px;">
                <path d="M58.7442 59.5633L46.4651 68.332L32.7907 50.377L19.5349 68.332L6.97674 59.5633L20.3721 40.634L0 34.2314L4.60465 20.1736L24.5581 26.5761V3.33203H41.0233V26.5761L60.9767 20.1736L66 34.2314L45.3488 40.634L58.7442 59.5633Z" fill="#18181b"/>
                <path d="M117.987 55.1217H129.049C140.689 55.1217 145.691 47.8108 145.691 33.3812C145.691 18.9517 140.689 13.1798 127.895 13.1798H117.987V55.1217ZM131.743 65.992H105V2.11719H129.723C147.038 2.11719 159.351 13.9494 159.351 33.3812C159.351 52.813 148.289 65.992 131.743 65.992Z" fill="#18181b"/>
                <path d="M192.939 41.8466L184.955 16.4505H184.859L176.682 41.8466H192.939ZM215.353 65.992H200.827L196.787 52.813H173.219L168.698 65.992H154.557L177.355 2.11719H192.843L215.353 65.992Z" fill="#18181b"/>
                <path d="M266.393 65.992H216.467V54.6408L249.655 13.276H217.429V2.11719H266.393V12.6027L233.013 54.6408H266.393V65.992Z" fill="#18181b"/>
                <path d="M319.283 65.992H269.356V54.6408L302.544 13.276H270.318V2.11719H319.283V12.6027L285.902 54.6408H319.283V65.992Z" fill="#18181b"/>
            </svg>
        ...
        <div class="footer">
            <svg xmlns="http://www.w3.org/2000/svg" width="100" height="10" viewBox="0 0 708 69" fill="none" style="margin-bottom: 15px;">
            ...
            <p style="color: #52525b; margin-top: 10px;">No respondas a este email</p>
        </div>
    </div>
</body>
</html>"""
...
def send_project_closed_email_multi(recipients: List[str], project_name: str, project_code: str, responsible_name: str, tickets_count: int, total_amount: float, excel_bytes: bytes, excel_filename: str) -> bool:
    """Envía email con Excel adjunto a múltiples destinatarios - CON ESTILO DE LA APP"""
    try:
        ...
        # Usar template con estilo de la app
